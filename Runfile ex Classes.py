...
f = risk_model['factor_betas'].loc[alpha_vector_index].values.T * weights
X = risk_model['factor_cov_matrix']
S = np.diag(risk_model['idiosyncratic_var_vector'].loc[alpha_vector_index].
            values.flatten())
...
constraints = [risk <= risk_cap ** 2,
               risk_model['factor_betas'].values.T @ weights <= factor_max,
               risk_model['factor_betas'].values.T @ weights >= factor_min,
               cvx.sum(weights) == 0,
               sum(cvx.abs(weights)) <= 1,
               cvx.min(weights) >= weights_min,
               cvx.max(weights) <= weights_max]

obj = cvx.Maximize(alpha_vector.values.T @ weights)
    
prob = cvx.Problem(obj, constraints)
prob.solve(max_iters=500)

optimal_weights = np.asarray(weights.value).flatten()
optimal_weights = pd.DataFrame(data=optimal_weights, 
...
optimal_weights_1 = np.asarray(weights.value).flatten()
optimal_weights_1 = pd.DataFrame(data=optimal_weights_1, 
                           index=alpha_vector.index)
...
# =============================================================================
# Optimization with strict factor exposures
# =============================================================================
...
obj = cvx.Minimize(cvx.norm(weights 
                             - cvx.reshape(x_star,weights.shape), 2))

prob = cvx.Problem(obj, constraints)
prob.solve(max_iters=500)

optimal_weights_2 = np.asarray(weights.value).flatten()
optimal_weights_2 = pd.DataFrame(data=optimal_weights_2, 
                           index=alpha_vector.index)
